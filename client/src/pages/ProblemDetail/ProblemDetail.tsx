...
export function ProblemDetail({ isloggedIn }: LoginProps) {
  ...
  const [panelMode,   setPanelMode]   = useState<PanelMode>("testcase");
  ...
  useEffect(() => {
    if (!id) return;
    axios.get(`${env.backendUrl}/api/question/${id}`)
      .then(r => setQuestion(r.data.question))
      .catch(console.error);
  }, [id]);
  ...
  const submitHandler = async () => {
    ...
    try {
      const res = await axios.post(`${env.backendUrl}/api/question/submit`, { questionId: id, code, language }, { withCredentials: true });
      setPanelMode("submit");
      setSubmitResult(res.data);
      const subRes = await axios.post(`${env.backendUrl}/api/submission/`, {
        questionId: id, code, language, status: res.data.status, title: question?.title
      }, { withCredentials: true });
      setSubmissionId(subRes.data.submission._id);
    } catch (err: unknown) {
      if (axios.isAxiosError(err) && err.response?.status === 500) setShowMaintenance(true);
    } finally {
      setIsRunning(false);
    }
  };
  ...
  return (
    <>
      ...
      <motion.div className="ws-layout" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.3 }}>

        {/* ── Left panel: Problem description ── */}
        <div className="ws-left">
          <div className="ws-left-header">
            <div className="ws-problem-meta">
              <span className="ws-problem-num">{questionNumber}.</span>
              <h1 className="ws-problem-title">{question?.title}</h1>
            </div>
            <div className="ws-badges">
              ...
              <span className="ws-acceptance">~{successRate}% acceptance</span>
            </div>
          </div>

          <div className="ws-left-body">
            <div className="ws-section">
              <p className="ws-description">{question?.description}</p>
            </div>

            {question?.example && (
              <div className="ws-section">
                <h3 className="ws-section-title">Example</h3>
                <div className="ws-example">
                  <div className="ws-example-row">
                    <span className="ws-example-label">Input:</span>
                    <code className="ws-example-val">{question.example.input}</code>
                  </div>
                  <div className="ws-example-row">
                    <span className="ws-example-label">Output:</span>
                    <code className="ws-example-val">{question.example.output}</code>
                  </div>
                  {question.example.explanation && (
                    <div className="ws-example-row">
                      <span className="ws-example-label">Explanation:</span>
                      <span className="ws-example-explain">{question.example.explanation}</span>
                    </div>
                  )}
                </div>
              </div>
            ...
            {question?.constraints && question.constraints.length > 0 && (
              <div className="ws-section">
                <h3 className="ws-section-title">Constraints</h3>
        ...
        <div className="ws-right">
          <div className="ws-editor-toolbar">
            <div className="ws-lang-select-wrap">
              <FiChevronDown size={13} className="ws-lang-chevron" />
              <select
